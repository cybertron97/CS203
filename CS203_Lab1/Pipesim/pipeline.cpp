...
bool Pipeline::hasDependency(void) {
	...
	if (!forwarding) {
		...
		// Check if value in WB is passable to the EXEC code.
		// If it isn't ready then it is dependent must be blocked.
		if ((pipeline[EXEC].inst->dest != -1) &&
			(pipeline[EXEC].inst->src1 == pipeline[WB].inst->dest ||
				pipeline[EXEC].inst->src2 == pipeline[WB].inst->dest))
		{
			return true;
		}

		// Check if value in MEM is passable to the EXEC code.
		// This is only allowable on a LW operation.
		// If it isn't ready then it is dependent must be blocked.
		if ((pipeline[EXEC].inst->dest != -1) && (pipeline[EXEC].inst->type == LW) &&
			(pipeline[EXEC].inst->src1 == pipeline[MEM].inst->dest ||
				pipeline[EXEC].inst->src2 == pipeline[MEM].inst->dest))
		{
			return true;
		}
	}
